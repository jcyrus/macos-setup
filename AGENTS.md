...
Run `./install.sh` in the terminal and monitor stdout/stderr in real-time.
...
| **`brew bundle` Partial Failure (`Some Brewfile entries failed...`)** | Network timeout, formula/cask conflict, broken download URL, or missing tap. | 1. Run `brew update`.<br>2. Parse failed entries from `install.sh` log.<br>3. Retry individual failures: `brew install <formula>` or `brew install --cask <cask>`.<br>4. Re-run `brew bundle --file=./Brewfile`. |
...
Inspect the output line-by-line for `warn` and `FAIL` indicators.

#### Doctor Issue Auto-Fix Rules

1. **Dangling Symlinks (`FAIL: dangling symlink -> ...`)**:
   - Cause: Target directory/file missing or moved.
   - Fix: Re-create symlinks pointing to repository source files:
     - Neovim: `ln -sfn "$REPO_DIR/nvim" "$HOME/.config/nvim"`
